...
```sh
tar xzf massa_TEST.23.1_release_linux.tar.gz
```
...
- pour l'utiliser : **wget https://github.com/massalabs/massa/releases/download/TEST.23.1/massa_TEST.23.1_release_linux.tar.gz** Il faudra remplacer les **XX.X** par la version recherchée

## 2. Décompression de l'archive

Si **tar** n'est pas présent, on l'installe avec **sudo apt install tar**

On utilise **tar** sur notre archive : **tar xzf massa_TEST.23.1_release_linux.tar.gz** ou **XX.X** est le numéro de la version.
